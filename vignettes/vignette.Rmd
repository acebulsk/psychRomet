...
df1 <- data.frame(T_C = seq(from = -40, to = 40, by = 5)) %>%
  mutate(
    ...
    W = humidity_ratio_p(e_sat, p_atm), # aka mixing ratio, g/kg
    p_air = density_dry_air(T_C, p_atm), # kg / m3
    # p_air_virtual = density_moist_air_virtual(T_C, p_atm, W) / 1000, # kg / m3 using virtual temperature
    p_wv = absolute_humidity(e_sat, T_C) * 1000, # aka water vapour density # g / m3
