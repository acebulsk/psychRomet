...
units <- c("$^{\\circ}C$", "$\\%$",  "$kPa$", "$kPa$", "$g/kg$", "$g/kg$", "$kg/m^3$", "$g/m^3$", "$g/m^3$", "$^{\\circ}C$", "$^{\\circ}C$", "$^{\\circ}C$")
...
df1 <- data.frame(T_c = seq(from = -40, to = 40, by = 5)) %>%
  mutate(
    ...
    p_wv = absolute_humidity(T_c, rh) * 1000, # aka water vapour density # g / m3
    T_d = dew_point_temp_e_act(e_act), # Celsius
    T_w = wet_bulb_empirical(T_c = T_c, e_act = e_act, e_sat = e_sat)
    )

df1$T_w_iter <- ice_bulb_iter(df1$T_c, df1$rh)
...
df2$T_w_iter <- ice_bulb_iter(df2$T_c, df2$rh)
